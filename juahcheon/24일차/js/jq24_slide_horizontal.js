...
/*
    ...
    // 순서가 밀리기 때문에 첫번째 장을 보여주는 상태로 세팅하기 위해 전체적으로 너비 한개만큼 당겨줌
    ...
    setInterval(function(){
        ...
        $("#wrap").animate({
            ...
            // #wrap 의 margin-left 값을 변경
            // parseInt(값) : 값을 정수로 변환해주는 JS 함수(값이 문자로 시작될 경우를 제외하고 숫자로 변경, 소수점 제거)
            // number("문자형 숫자") : 숫자로 형변환(소수점이 있을 경우 그대로 변환)
            // $("#wrap").css("margin-left") 값이 "-600px" 이기 때문에 뒤쪽에 있는 -600px과 계산이 안되기 때문에 정수로 변환해서 계산을 해줌.(- 600 - 600 + "px")
        ...
        .animate({변경할 css 속성}, 실행되는 시간, 가속도, 애니메이션이 종료 후 바로 실행될 함수);
        실행되는 시간 : 밀리초
        가속도(생략 가능) : 기본적으로 "swing", "linear"
        콜백함수(생략 가능) : 작업이 끝나고 난 뒤 바로 실행할 함수

        // console.log( $("#wrap").css("margin-left") );    // 600.006 (애매한 숫자: px때문에)
    ...
    */
    ...
    var i = 0;
    // 마진 값을 늘려주기 위한 카운터 변수 0, 1, 2, 0, 1, 2...

    function autoSlide() {
        if(i < slideLength -1) {
            // 첫번째 이미지는 이미 보여지고 있는 상태였기 때문에 움직이는 횟수에서 1을 뺀다.
            // 1회전: i=0 < 3-1=2 (true)
            // 2회전: i=1 < 3-1=2 (true)
            // 3회전: i=2 < 3-1=2 (false)
    ...
    // 한 개의 함수만 반복한다면 함수명만 괄호없이 작성
    setInterval(autoSlide, 10000);

    실행함수를 바로 작성한다면 그 안에서 함수를 다시 호출할 때 함수명(); 괄호를 넣어야 함.
    setInterval(function(){
        autoSlide();
    }, 10000);
    */
    ...
    function autoSlide() {
        ...
        // 선택자.eq(인덱스번호) : 인덱스 번호에 해당하는 요소를 선택하는 매서드
        // 선택자.siblings(); : 선택자를 제외한 나머지 형제 요소들
        // 1번 슬라이드 = 인덱스 번호 0번

        // 1회전 0번 슬라이드를 제외한 나머지 형제들(1, 2)을 fadeOut 시키고 0번은 fadeIn
