$(function(){
    ...
    var lastScrollTop = 0;
    var delta = 5;
    // 동작의 구현이 시작되는 위치 초기값 설정.
    var headerHeight = $("header").outerHeight();
    // 헤더의 높이값
    // outerheight() : padding, border, margin을 다 합한 값
    var didScroll;
    // 추후 사용을 위한 변수 설정.
    ...
    $(window).scroll(function(){
        ...
        didScroll = true;
        // 스크롤 이벤트가 발생하면 didScroll 변수에 true 저장.

        
        // 스크롤 시 요소가 페이드인 되면서 나타남
        $(".fadeInLeft").each(function(){
            // .each(); : 배열에 저장된 문서 객체만큼 인덱스에 하나씩 차례로 접근해서 매서드가 반복실행된다. JS에서의 for문과 비슷하다.

            var bottomOfElement = $(this).offset().top + $(this).outerHeight();
            // 현재 요소의 offset 높이값(페이지를 기준으로 위에서 얼마나 떨어져 있는지)

            var bottomOfWindow = $(window).scrollTop() + $(window).height();
            // window 객체의 수직 스크롤 이동 높이 + window 객체의 높이
    ...
    function hasScrolled() {
        ...
        // 설정한 delta 값보다 더 스크롤 되었는지 확인한다.
        // Math.abs(값) : 값을 절대값으로 변환. JS의 수학객체


        if( scrollTop > lastScrollTop && scrollTop > headerHeight ) {
        //  스크롤위치 >       0      && 스크롤위치 > 헤더높이
        //      0      >       0      &&      0     > 50
        // 스크롤을 한 번이라도 내리면 true
    ...
    setInterval(function(){
        ...
        // 스크롤 이벤트가 발생될 때마다 실행한다.
        if(didScroll) {
            hasScrolled();
            // 스크롤을 아래로 내렸는지 위로 올렸는지 판단해서 실행해주는 함수
            didScroll = false;
            // true 인 상태에서 다시 체크하기 위해 false로 되돌려줌.
        }
