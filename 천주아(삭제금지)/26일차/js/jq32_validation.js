$(function(){
    ...
    // 인풋태그 클릭시 레이블 이동
    // 레이블 태그를 placeholder같이 사용하고 싶을 때
    $(".form_group input").focus(function(){
        // .focus() : 포커스가 잡혔을 때 이벤트 매서드
        // 클릭했을 때도 포커스가 잡히기 때문에 포함된다.

        $(this).siblings("label").addClass("active");
        // 클릭한 인풋태그의 현재요소 중 label태그인 것
    ...
    // 이 경우 인풋태그에 내용을 입력하더라도 레이블이 되돌아와 겹쳐짐
    */

    $(".form_group input").blur(function(){
        
        if( $(this).val() == "" ) {
            // .val() : 인풋태그 안에 입력한 값을 반환하거나 변경해주는 매서드
    ...
    $("#all_agree").click(function(){
        ...
        if( $("#all_agree").prop("checked") ) {
            // #all_agree에 체크가 되어 있으면
            
            $(".agree_check:checkbox").prop("checked", true);
            // agree_check 클래스의 :checkbox 에 checked 상태를 true로 변경(체크한다.)
        } else {
            $(".agree_check:checkbox").prop("checked", false);
        }
        // .prop() : property - 블리언값으로 반환
        // .attr()와 비슷하나, .prop() 은 form 요소의 disabled, selected, checked 와 같은 속성값을 가져오거나 변경할 때 주로 사용
    ...
    // 아이디 유효성 검사하는 정규식 : 소문자 a-z, 대문자 A-Z, 숫자 0-9, 언더스코어(_), 하이픈(-)이 올 수 있고 최소 3자, 최대 10자 이하

    var reg_pw = /^[a-z0-9_-]{6,18}$/;
    // 패스워드 유호성 검사 정규식 : 소문자 a-z, 숫자 0-9, 언더스코어(_), 하이픈(-)이 올 수 있고 최소 6자, 최대 18자 이하
    ...
    $("form").submit(function(e){
        // .submit() : form태그에 submit 이벤트가 일어나면(확인 버튼을 누르면)
        // 폼태그 내부의 값을 모두 입력한 후 엔터를 치면 기본적으로 submit 이벤트가 실행된다.
        
        e.preventDefault;
        // submit 이벤트의 기본 내장 이벤트로 action 속성값으로 넘어가기 때문에 이를 방지하기 위해 추가


        // 아이디 빈 칸인지 여부, 유효성 검사
        if( $("#user_id").val() == "" ) {
            // 아이디 칸의 value 값이 빈 문자열인 경우(아무것도 입력하지 않았을 때)
            ...
            return false;
            // 아이디가 입력이 안되어 있으면 false를 반환해 submit 이벤트 중지

        } else if( !reg_id.test( $("#user_id").val() ) ) {
            // 아이디 유효성 검사를 통과하지 못하면
            // .test() : 문자열 일치여부를 테스트하는 매서드. 일치하면 true, 불일치하면 false를 반환.
        ...
        } else if( !reg_pw.test( $("#user_mane").val() ) ) {
        ...
        } else if( !reg_pw.test( $("#user_email").val() ) ) {
        ...
        } else if( !reg_pw.test( $("#user_phone").val() ) ) {
    ...
    $("#user_id").keyup(function(){
        ...
        if( $("#user_id").val().length < 3 ) {
            ...
            $(this).next("span").text("아이디는 3자 이상 입력해주세요.");
    ...
    $("#user_pw").keyup(function(){
        ...
        if( $("#user_pw").val().length < 6 ) {
            ...
            $(this).next("span").text("패스워드를 18자 이하로 입력해주세요.");
